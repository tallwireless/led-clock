...
class RunText(SampleBase):
    ...
    def run(self):
        ...
        while True:
            ...
            offset=7
            for text in my_text:
                graphics.DrawText(offscreen_canvas, stationary_font, stationary_pos, offset, self.textColor, text)
                offset+=10
            l = graphics.DrawText(offscreen_canvas, scrolling_font, scrolling_pos, offset, self.textColor, scrolling_text[st_index])
            scrolling_pos -= 1
             
            if (scrolling_pos + l < 0):
                st_index += 1
                if st_index >= st_len:
                    st_index = 0
                scrolling_pos = offscreen_canvas.width
